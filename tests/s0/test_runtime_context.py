...
from tests.s0.runtime_fixtures import RAW, complete_intake, deps, initial_state
...
async def test_runtime_context_n0_ownership과_checkpoint_leakage():
    ...
    result = await graph.ainvoke(
        initial_state(), config, context=ReviewRequestContext(intake=complete_intake())
    )
...
def test_intake_review_topology와_n0만_raw_text를_읽는_architecture():
    assert VERTICES == (
        "n0",
        "n1",
        "n2",
