...
- 하나의 문장에 AND/그리고/또한/및/하면서 등으로 서로 독립적으로 확인할 수
  있는 proposition이 연결되어 있으면 각 proposition을 별도 unit으로 만든다.
- 예: "최근 반도체 업황과 회사 실적이 좋아지고 있다"는 업황 unit과 실적
  unit으로 나눈다. 사용자가 말하지 않은 대상·기간·수치·비교 대상을 보태지
  않는다.
- Slot 7의 복합 우려도 같은 규칙을 적용한다. A와 B를 각각 만들었다면
  A+B 전체를 같은 의미의 세 번째 unit으로 다시 만들지 않는다.
- 자식 unit을 만들었을 때 단순히 그 자식들을 합친 부모 표현을 추가하지 않는다.
  각 unit의 text_span과 span_offset은 원문에서 그대로 가져오고 provenance를
  유지한다.
